...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def _install_package(self):
        """
        Run pip install <package> and stream the result back.
        Request: { "package": "ezdxf" }
        Response: { "ok": true, "output": "...", "returncode": 0 }
        """
        try:
            ...
            # Whitelist — only allow known safe packages
            allowed = {'ezdxf', 'pillow', 'Pillow'}
            if package not in allowed:
                self._json_response(400, {'error': f'Package "{package}" not in allowed list: {allowed}'})
                return
            ...
            self._json_response(500, {'error': 'pip install timed out after 120s'})
        except Exception as e:
            self._json_response(500, {'error': str(e)})
